...
const TraceStepItem: React.FC<{ step: TraceStep }> = ({ step }) => {
  return (
    <div className="flex items-center gap-2 text-xs text-[rgb(var(--teal))]/80 mb-1.5 font-medium animate-in fade-in slide-in-from-left-2 font-mono">
...
export const AgentChat: React.FC<AgentChatProps> = ({ messages, onSendMessage, isProcessing, demoInputValue = '' }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[rgb(var(--deep-blue))]/30 relative">
      {/* Messages Area */}
          ...
          <div key={msg.id} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} animate-in fade-in slide-in-from-bottom-2`}>
            ...
            <div className={`max-w-[85%] ${msg.role === 'user' ? 'text-right' : 'text-left'}`}>
              ...
              {msg.role === 'agent' && msg.traces && msg.traces.length > 0 && (
                <div className="mb-4 pl-2 border-l-2 border-[rgb(var(--ink-muted))]/20">
              ...
              {/* Message Content */}
              {msg.role === 'user' ? (
                <div className="px-5 py-3 bg-[rgb(var(--deep-blue))] text-white rounded-[14px] rounded-tr-sm text-sm border border-[rgb(var(--ink-muted))]/20 shadow-sm">
                  {msg.content}
                </div>
              ) : (
      ...
      {/* Input Area */}
      <div className="p-4 border-t border-[rgb(var(--ink-muted))]/20 bg-[rgb(var(--deep-blue))]/50 backdrop-blur-sm">
        <form onSubmit={handleSubmit} className="relative max-w-3xl mx-auto w-full">
           <input
            ...
            className="w-full bg-[rgb(var(--deep-blue))]/50 text-white text-sm rounded-full py-3 pl-5 pr-12 focus:outline-none focus:ring-1 focus:ring-[rgb(var(--teal))] border border-[rgb(var(--ink-muted))]/20 placeholder-[rgb(var(--ink-muted))]/50 transition-all shadow-lg"
          ...
          <button
            ...
            className={`absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-[rgb(var(--teal))] text-white rounded-full transition-all transform 
              ${(!inputValue.trim() || isProcessing) ? 'opacity-50 scale-90' : 'hover:bg-[rgb(var(--teal))]/80 scale-100 shadow-md'}
            `}
          >
